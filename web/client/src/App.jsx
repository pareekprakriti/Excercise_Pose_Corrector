  ...
  return (
    <div className="center-wrap">
      <div className="hero-section">
        ...
        <p className="hero-sub">Real-time AI exercise correction powered by machine learning.<br />Instant feedback, every rep.</p>
...
function SignIn({ onNavigate, onLogin }) {
  ...
  async function submit(e) {
    ...
    try {
      ...
      // ← now passes user_id too
      onLogin({ email: data.email, name: data.name, user_id: data.user_id });
    } catch (err) {
...
function Dashboard({ user, onLogout }) {
  ...
  const [sessionId, setSessionId] = useState(null);   // ← tracks current session
  ...
  const repCountRef = useRef(0);   // ← keep ref in sync for stopStream closure

  const ex = EXERCISES.find(e => e.id === activeEx);

  // ── Start session in DB when exercise selected ──
    ...
    if (user.user_id) {
      try {
        const res = await fetch("http://localhost:8000/api/session/start", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ user_id: user.user_id, exercise_type: id, mode: "live" }),
        });
        const data = await res.json();
        if (data.session_id) {
  ...
  // ── End session in DB ──
  async function endSessionInDB(sid, totalReps) {
    if (!sid) return;
    try {
      await fetch("http://localhost:8000/api/session/end", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ session_id: sid, total_reps: totalReps, avg_accuracy: 0 }),
      });
  ...
  function stopStream() {
    ...
    endSessionInDB(sessionId, repCountRef.current);
    setStreaming(false);
    setFeedback(null);
  }
  ...
  async function submitVideo() {
    ...
    try {
      const url = `http://localhost:8000/api/video/upload?type=${activeEx}&user_id=${user.user_id || ""}`;
      const res = await fetch(url, { method: "POST", body: formData });
      const data = await res.json();
      setUploadResult(data);
      // Reload video preview so it doesn't show black screen after analysis
      setTimeout(() => {
  ...
  function startMediaPipe(exId) {
    ...
    Promise.all([
      ...
      pose.onResults((results) => {
        ...
        // ← passes session_id in query param
        const sid = sessionId;
        fetch(`http://localhost:8000/api/video/stream?type=${exId}&session_id=${sid || ""}`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ landmarks: lms, counter: repCountRef.current }),
        })
          .then(res => res.json())
          .then(data => {
            window._isSending = false;
            ...
            if (data.counter !== undefined) {
              repCountRef.current = data.counter;
              setRepCount(data.counter);
            }
          })
          ...
          <div className="session-layout">
              ...
              {mode === "live" ? (
                <>
                  ...
                  <div className="cam-controls">
                    ...
                    <button className="btn outline" onClick={() => { setRepCount(0); repCountRef.current = 0; }}>
                      Reset Count
                    </button>
                  </div>
                </>
                  ...
                  <div className="cam-controls">
                    ...
                    {!uploadPreview && (
                      <button className="btn primary" onClick={() => fileInputRef.current?.click()}
                        style={{ flex: 1 }}>📁 Choose Video File</button>
                    )}
                  </div>
            ...
            <div className="session-sidebar">
              ...
              <div className="side-card">
                ...
                {sessionId && <div className="api-hint" style={{marginTop:6}}>Session ID: {sessionId}</div>}
              </div>
            </div>
          </div>
        )}
