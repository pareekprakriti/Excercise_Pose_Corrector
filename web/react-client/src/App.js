  ...
  return (
    <div className="center-wrap">
      <div className="hero-section">
        ...
        <p className="hero-sub">Real-time AI exercise correction powered by ML models.<br />Instant feedback, every rep.</p>
...
function Dashboard({ user, onLogout }) {
  ...
  function startMediaPipe(exId) {
    ...
    Promise.all([
      ...
      // Track last known posture color
      window._postureColor = "#00d4ff"; // default cyan until first response
      ...
      pose.onResults((results) => {
        ...
        // Always draw skeleton with last known color (no lag)
        drawSkeleton(lms, window._postureColor);

        // Send landmarks to Django (non-blocking)
        ...
        fetch(`http://localhost:8000/api/video/stream?type=${exId}`, {
          ...
          .then(data => {
            window._isSending = false;
            // Update color based on Django response
            window._postureColor = data.posture_ok ? "#00ff87" : "#ff4757";
            setFeedback({
              posture_ok: data.posture_ok,
                    ...
                    <div className="upload-result">
                      {uploadResult.error ? (
                        ...
                        <>
                          ...
                          <div className={`feedback-status ${uploadResult.posture_ok ? "ok" : "bad"}`}
                          ...
                          {uploadResult.accuracy !== undefined && (
                            <>
                              <div className="acc-row">
                                <span>ACCURACY</span>
                                <span className="acc-val">{uploadResult.accuracy}%</span>
                              </div>
                          ...
                          {uploadResult.counter !== undefined && (
                            <div style={{ marginTop: 14, textAlign: "center" }}>
                              <div style={{ fontSize: 11, color: "var(--muted)", letterSpacing: 2,
                                textTransform: "uppercase", marginBottom: 6 }}>Reps Detected</div>
                              <div style={{ fontFamily: "'Bebas Neue'", fontSize: 48,
                                color: "var(--green)", textShadow: "0 0 20px rgba(0,255,135,.4)" }}>
